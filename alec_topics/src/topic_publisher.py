#!/usr/bin/env python
import rospy
from std_msgs.msg import Int32 # standard int from standard messages package

# Setup: initialize node, register topic, set rate
rospy.init_node( # initialize node
  'topic_publisher' # node default name given here (default name)
)
pub = rospy.Publisher( # register topic w/roscore
  'counter', # topic name
  Int32, # topic type
  queue_size=5 # queue size, buffer for topic
)
